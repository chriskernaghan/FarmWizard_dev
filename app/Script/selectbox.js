...
function updateBMCount() {
	var selected = 0;
        var obj = document.forms[0].ListBox;
	for (var i=0; i<obj.options.length; i++) {
		if(obj.options[i].selected == true) {
			selected += 1;
		}
	}
	document.getElementById("SelectedCount").firstChild.data  = selected;
}
...
// -------------------------------------------------------------------
// selectNextMatchingOption(select_object,regex)
//  This function selects first options that matches the regular expression
//  passed in. Only one entry can be seleted at a time
// -------------------------------------------------------------------
function selectNextMatchingOption(obj,regex) {
        var re = new RegExp(regex);
		if (!hasOptions(obj)) { return; }
        var start = 0;
		...
		for (var i=start; i<obj.options.length; i++) {
			if (re.test(obj.options[i].text)) {
